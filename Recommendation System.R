  ...
  navbarPage(
    ...
    navbarMenu(
      ...
      tabPanel(title = "Using Gower's Distance Metric",
               ...
               sidebarLayout(
                 sidebarPanel(
                   helpText("In this section, we'll need to enter the title of 3 TV series or Movies on Netflix that you're interested in"),  
                   tags$br(),
                   helpText("We'll use this as the basis to recommend similar movies to your liking"),
                   tags$br(),
...
server = function(input, output){
  ...
  output$gower_entry_1_check = renderText({
    if (input$gower_entry_1 %in% netflix_gower_combine$title) {
      as.character("Yes, the title can be used")
    } else {
      as.character("Something went wrong here.")
    }
  })
